...
#[cfg(windows)]
static DBC_INITIALIZED:  OnceLock<()> = OnceLock::new();

#[cfg(windows)]
unsafe fn register_rq_functions() {
    // DBC init is expensive and only needs to happen once.
    DBC_INITIALIZED.get_or_init(|| {
        dbc::init_schema_registry();
        dbc::init_mpq_list();
    });
    // Lua registrations must happen on every hook call because the game
    // reinitializes the Lua state when PLAYER_LOAD_SCRIPT_FUNCTIONS runs,
    // wiping any functions registered during the earlier glue hook.
    use scripts::*;
